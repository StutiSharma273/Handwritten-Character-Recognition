...
class TestClass():
    ...
    def readImage(self,image_uri,user_interface):
        ...
        #sorted_contours = sorted(contours, key=lambda ctr:cv2.boundingRect(ctr)[1]*self.image.shape[1] + cv2boundingRect(ctr)[0])
        ...
        for c in sorted_contours:
            ...
            #Creating a rectangle around the digit in the original image(for diaplying the digits fetched via contours)  
            cv2.rectangle(self.image,(x,y),(x+w,y+h),color = (0,255,0),thickness=2)

            #Cropping out the digitfrom the image corresponding to the current contoursin the for loop
            digit = thresh[y:y+h,x:x+w]

            #Resizing that digit to (18,18)
            resized_digit = cv2.resize(digit,(20,20))

            #Padding the digit with 5 pixels of black colour(zeroes) in each sid to finally produce the image of (28,28)
            ...
            #cv2.putText(self.image,str(chr(ord('A')+ np.argmax(prediction))),(x,y-10),cv2.FONT_HERSHEY_SIMPLEX,0.6,(36,255,12))
            ...
            #self.strFinal +=str(chr(ord('A')+np.arfmax(prediction)))
